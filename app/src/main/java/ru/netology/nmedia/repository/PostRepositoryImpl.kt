...
class PostRepositoryImpl(private val dao: PostDao) : PostRepository {
    ...
    //сохранение со вложением
    override suspend fun saveWithAttachment(post: Post, upload: MediaUpload) {
        try {
            //загружаем на сервер картинку. Сервер в ответ выдает адрес картинки
            val media = upload(upload)
            println("Adress of image ${media.id}")
    ...
    override suspend fun upload(upload: MediaUpload): Media {
        try {
            //передается имя файла, asRequestBody - преобразовывает файл в последовательность байт
